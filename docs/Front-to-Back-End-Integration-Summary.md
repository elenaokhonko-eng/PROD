...
- "Layer" in this document refers to the **frontend product layer** the user is in when the call is triggered. Layers 1 and 2 map 1:1 to the current backend tiers and entitlement values in `user_entitlements` / `case_entitlements`; Layer 3 is the post-Tier-1 / Tier-2 surface:
  ...
  - **Layer 3 = Tier 2 post-report surface** — human-in-the-loop FIDReC contact form plus specialist consult / case-pack commerce. The contact form has no edge function and writes to the contact-requests storage table + notification email to Dance. The persistent WhatsApp link is a third-party `wa.me` entry point in the root layout. Slice 8 adds the SGD 188 `fidrec_tier2_pack` and separate SGD 99 `human_consult_30m` checkout on the same surface (§10.6).
...
**Action.** The file picker uses `accept="application/pdf,image/png,image/jpeg"` and an explicit client-side MIME check (not just extension) before calling Supabase Storage. Rejections show an inline message: *"We accept PDF, PNG, or JPEG. Please convert your file and try again."*

**State Machine mention.** Upload node is annotated: *"Accepted types: PDF, PNG, JPEG. Client-side MIME validation rejects everything else **before** upload to Storage. DOCX remains gated until its end-to-end fixture passes."*

> **Resolved MVP gate (§10.2):** DOCX is excluded from the production whitelist until `evidence_processed_v2` handles its end-to-end fixture cleanly. The current list is PDF / PNG / JPEG.
...
**Status.** The production MIME whitelist in §9.6 is PDF / PNG / JPEG. DOCX remains excluded because Masha's existing test cases did not cover it and its behaviour is unverified on the production path.
...
| FIDReC case pack | SGD 188 | Case-linked executive summary and chronology/timeline with PDF and Markdown downloads. Product key `fidrec_tier2_pack`. |
...
- Paid add-ons reuse the existing Stripe checkout + webhook pattern with explicit product metadata distinguishing the SGD 188 FIDReC pack and SGD 99 consult flows. The webhook branch for these add-ons must not call the Layer 2 decision/report worker.
