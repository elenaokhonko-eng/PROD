...
- **Layer 3 / Tier 2 — post-report FIDReC handoff + specialist commerce.** Same surface after the Tier-1 report. Contact form: user enters age + employment status + two FIDReC-qualification checkboxes + optional message; server snapshots `amount_lost_sgd` + `financial_institution` from the latest extract; one `upsert` into contact-requests storage + email to Dance. No edge function on the contact path. The root-layout WhatsApp `wa.me` link remains available on every route, and Slice 8 adds the SGD 188 `fidrec_tier2_pack` plus the separate SGD 99 `human_consult_30m` offer on this surface.

---

## 0. Current Slice 8 Update (2026-07-12)

Older Slice 8 notes described the Layer 3 / Tier 2 commerce surface as SGD 99 consult plus SGD 800 case pack. The current product split is:

- **FIDReC Tier 2 pack:** SGD 188, case-linked, generated from the completed Tier 1 case material. Output is an executive summary plus chronology/timeline, downloadable as PDF and Markdown.
- **Human consult:** SGD 99, separate from the pack. It is a human advice/direction call. It must eventually be integrated with the case by recording the call, transcribing it, and adding the result to the case narrative set as a client consultation narrative. The exact backend workflow is pending Masha's design.
- **Stripe catalogue correction:** the old SGD 99 Stripe price that previously backed Tier 1 is now the Human in the Loop Consultation price. Tier 1 is customer-facing **FI Pack**, SGD 18, product key `self_serve_report`, env `STRIPE_PRICE_ID_SELF_SERVE_REPORT_SGD`, price `price_1TsLZdFp6sSKMUXXz5xEbxrA`. Tier 2 is **FIDReC Pack**, SGD 188, product key `fidrec_tier2_pack`, env `STRIPE_PRICE_ID_FIDREC_TIER2_PACK_SGD`, price `price_1TsLY5Fp6sSKMUXXdHFwRsny`. Consult is `human_consult_30m`, env `STRIPE_PRICE_ID_HUMAN_CONSULT_30M_SGD`, price `price_1SLOYUFp6sSKMUXXsTXlLdcT`.
- **State-machine rule:** `escalation_pack` is a valid post-report Layer 3 / Tier 2 state. It must not route the user back into the old self-serve buy-report flow.
- **Slice 8 test case:** `688154e7-9cda-47ef-9cff-a27581766c3a`.

The frontend should keep the existing Layer 3 contact form and WhatsApp entry points, add the Tier 2 pack preview/download path, and expose the consult CTA as a separate offer until the recording/transcription workflow is defined.
...
| R7 | **Evidence upload MIME whitelist: PDF, PNG, JPEG.** Reject everything else client-side before Storage. DOCX remains gated until its end-to-end fixture passes. | IS §8.1 gotcha 4; IS §9.6 |
...
Slice 8 extends this surface with the following additional states:

- **T2-PackOffer:** shown after Tier 1 report completion when the user can buy the SGD 188 FIDReC pack.
- **T2-PackCheckout:** calls Stripe checkout with `product_key = fidrec_tier2_pack`.
- **T2-PackReady:** shown when `escalation_pack` entitlement is present; renders executive summary and chronology/timeline, with PDF and Markdown downloads.
- **T2-PackFailed:** shown when the JSON/export route fails or required Tier 2 artefacts are missing.
- **T2-ConsultOffer:** separate SGD 99 human consult offer with `product_key = human_consult_30m`; recording/transcription/narrative insertion waits for Masha's backend workflow.

| State | What the user sees | What is running | Reads | Writes |
|---|---|---|---|---|
| **L3-FormFilling** | Human-in-the-loop form plus specialist recommendation copy. **Auto-filled, editable:** first name, last name, email, phone (from Clerk user profile). **User-entered:** `age` (int 13–120), `employment_status` (radio: professional / retiree / student / other), `thirty_days_since_last_fi_reply` (checkbox Y/N), `fi_issued_final_response` (checkbox Y/N), optional `message` (≤ 500 chars). **Read-only context card** above the form showing the snapshot values Dance will see: financial institution, reported loss (SGD), case ID. Hidden: `case_id` is carried in the route context; `user_id` is never sent (column `DEFAULT auth.uid()`). Root-layout WhatsApp remains visible; Slice 8 adds the SGD 188 Tier 2 pack and separate SGD 99 consult offers on this surface. | Idle. | Clerk user profile client-side; the snapshot values displayed in the read-only card are also read via a user-scoped Supabase query on `cases` + latest `case_extract_runs.extract_json`. | — |
...
- Keep the existing root-layout WhatsApp `wa.me` link available on every route. Do not add a duplicate global widget. On this Layer 3 / Tier 2 surface, include Scam and Fraud Specialist recommendation copy; Slice 8 adds the SGD 188 `fidrec_tier2_pack` path plus the separate SGD 99 `human_consult_30m` offer.
...
| 17 | Evidence upload component with MIME whitelist (PDF / PNG / JPEG). | IS §9.6 | S1-EvidenceFirstUpload, GL-Uploading, L2-UpgradeScreen. |
...
- [ ] Upload component rejects anything outside PDF / PNG / JPEG at the file-picker step (R7); DOCX is not advertised until its fixture passes.
...
If Priya later upgrades to Tier-1 and finishes her paid self-serve report, the report viewer shows a *"Need help escalating to FIDReC? → Get help from a specialist"* CTA. Clicking it takes her to **L3-FormFilling** — a human-in-the-loop form on the Layer 3 / Tier 2 surface. Her first name, last name, email, and phone are **auto-filled from her Clerk profile** (editable). She then enters her **age** (e.g. 62), picks an **employment status** (`retiree`), ticks the two FIDReC-qualification booleans, and optionally writes a one-liner. Above the form sits a read-only context card showing what Dance will see on his side: financial institution, reported loss, and case ID from the latest `case_extract_runs.extract_json`, not from form inputs. On submit, the browser POSTs to `/api/contact-requests` with user-entered fields only. The same surface also keeps the persistent WhatsApp entry point and, after Slice 8, can show the SGD 188 `fidrec_tier2_pack` with PDF/Markdown downloads plus the separate SGD 99 `human_consult_30m` offer. No edge function runs from the contact path, and LinkedIn / generic waitlist framing stays out.
...
- **2026-07-12** — **Slice 8 product correction.** Layer 3 and Tier 2 remain the same post-Tier-1 surface. Slice 8 now means SGD 188 `fidrec_tier2_pack` with executive summary + chronology/timeline + PDF/Markdown downloads, plus separate SGD 99 `human_consult_30m`. `escalation_pack` is a valid Tier 2 entitlement and must not route back to buy-report.
