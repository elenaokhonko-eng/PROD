...
- **Layer 3 — FIDReC handoff human-in-the-loop form.** *No backend tier, no `plan` value.* Single screen after the Tier-1 report. User enters age + employment status + two FIDReC-qualification checkboxes + optional message; server snapshots `amount_lost_sgd` + `financial_institution` from the latest extract; one `upsert` into contact-requests storage + email to Dance.
...
6. [Diagram 4 — Layer 3 terminal: FIDReC handoff contact form](#6-diagram-4--layer-3-terminal-fidrec-handoff-contact-form)
...
| R5 | **Validation reads are two-step via `get_case_eligibility.resolved_ids`.** Never query `v_latest_validation` or `case_validation_runs` by `case_id`. Note: validation rows themselves are populated by a **Postgres trigger on `case_extract_runs`** — they are not produced by a separate edge-function call. | IS §8.1 gotcha 6; IS §9.4; reconciliation doc §0.3 Q2 |
...
| R12 | **Layer 3 has no edge-function calls.** Human-in-the-loop contact form only, shown immediately after the Tier-1 report is generated. **User-entered fields:** first name, last name, email, phone, **age** (integer), **employment status** (`professional` / `retiree` / `student` / `other`), two FIDReC-qualification checkboxes (**≥ 30 days since last FI reply?**, **FI issued final response?**), optional message. **Auto-captured server-side, never trusted from the client:** `user_id` (column `DEFAULT auth.uid()`), `case_id` (validated via RLS-scoped ownership probe), `amount_lost_sgd` + `financial_institution` (snapshotted from the latest `case_extract_runs` row at insert time). One `upsert` (UNIQUE on `user_id + case_id`) into the contact-requests storage table via `/api/contact-requests`, plus one notification email to Dance. **Deprecated 2026-04-21 PM morning:** the specialist card with WhatsApp / LinkedIn CTAs and the "Escalation Pack — coming soon" waitlist framing. **Deprecated 2026-04-21 PM mid-afternoon:** the 4-field "name / email / phone / optional message" minimal form — expanded late-afternoon to capture FIDReC-qualification signals and demographics so Dance can triage at a glance. | IS §8.1 gotcha 3; IS §9.9, §10.5; reconciliation doc §0.4 |
...
sequenceDiagram
    ...
    rect rgb(230, 245, 220)
        ...
        FE->>SB: Upload to Storage + INSERT case_documents (status=pending)
    ...
    rect rgb(245, 225, 240)
        note over U,SB: Layer 3 — FIDReC handoff human-in-the-loop form (no edge function, R12)
...
| **S1-EvidenceFirstUpload** | Upload prompt with MIME whitelist copy (R7). Each upload shows a processing chip. No gap questions yet — the form waits for the first document to come back `ready`. | Realtime channel open on `case_documents` filtered by `case_id`. Server-route calls `POST /api/edge/evidence` per upload (canonical step 1 — `evidence_processed_v2`). | `case_documents_enriched` for per-doc verification + extractions. | `INSERT case_documents` (client) + edge-function writes `case_documents_content`, `case_document_verifications`, `case_document_chunks`, `case_document_extractions`. |
| **S1-GapLoop** | Two-column UI: left = gap questions, right = evidence upload. Entered after first doc is ready + minimum intake present; at that moment the frontend fires the **first** `POST /api/edge/extract`. | Same Realtime channel. Server route runs canonical step 2 (`run_case_extract_v4`) per gap answer (R11 clause b) and per new `evidence_processed_v2` success (R11 clause c). | Two-step read (R5): `rpc get_case_eligibility` → `case_extract_runs` + `case_validation_runs` by PK (validation rows populated by Postgres trigger — R5). | On gap answer: `INSERT case_intake (intake_type='gap_response')`. Edge function writes `case_extract_runs`; trigger writes `case_validation_runs`. |
| **GL-Submitting** | Save button spinner. | POST `/api/edge/extract`. | — | (edge function writes `case_extract_runs`; trigger writes `case_validation_runs`). |
| **GL-Processing** | Per-doc progress: `pending → parsing → verifying → chunking → extracting → ready/failed`. | Realtime subscription drives transitions (R8). Server route auto-chains `/api/edge/extract` on `status=ready` (R11 clause c). | `case_documents_enriched` view. | (edge function writes `case_documents`, `case_documents_content`, `case_document_verifications`, `case_document_chunks`, `case_document_extractions`). |
...
## 6. Diagram 4 — Layer 3 terminal: FIDReC handoff contact form (human-in-the-loop)

Terminal node. No edge function calls. Human-in-the-loop contact form shown **immediately after the Tier-1 report is generated** (entered via a CTA on `L2-ReportReady`: *"Need help escalating to FIDReC? → Get help from a specialist"*). Collects identity + age + employment status + two FIDReC-qualification checkboxes + optional message from the user; server-side snapshots `amount_lost_sgd` and `financial_institution` from the latest `case_extract_runs` row at insert time, so Dance's triage view always matches the number the user saw on their report (per IS §9.9).
...
### Layer 3 — per-state contracts

| State | What the user sees | What is running | Reads | Writes |
|---|---|---|---|---|
| **L3-FormFilling** | Human-in-the-loop form. **Auto-filled, editable:** first name, last name, email, phone (from Clerk user profile). **User-entered:** `age` (int 13–120), `employment_status` (radio: professional / retiree / student / other), `thirty_days_since_last_fi_reply` (checkbox Y/N), `fi_issued_final_response` (checkbox Y/N), optional `message` (≤ 500 chars). **Read-only context card** above the form showing the snapshot values Dance will see: financial institution, reported loss (SGD), case ID. Hidden: `case_id` is carried in the route context; `user_id` is never sent (column `DEFAULT auth.uid()`). | Idle. | Clerk user profile client-side; the snapshot values displayed in the read-only card are also read via a user-scoped Supabase query on `cases` + latest `case_extract_runs.extract_json`. | — |
...
### Layer 3 — binding rules reminder

- R12 — NO edge function calls. Ever. The only server-side call from Layer 3 is `/api/contact-requests` → Supabase `upsert` + email.
- No specialist card, no WhatsApp / LinkedIn CTAs, no "coming soon" waitlist framing (all deprecated 2026-04-21 PM morning).
...
| 15 | Two-step validation read helper (RPC + PK lookup). Remember: validation rows are populated by a **Postgres trigger on `case_extract_runs`**, not by a separate edge-function call (R5). | IS §9.4; reconciliation doc §0.3 Q2 | S1-GapLoop entry and every extract re-run completion. |
...
- [ ] Validation reads use `get_case_eligibility` + PK lookup, not `v_latest_validation` by `case_id` (R5). Grep for `v_latest_validation` — should be zero hits in `app/` and `lib/`. No code path calls a "validation function" — validation is a Postgres trigger on `case_extract_runs`.
...
- [ ] `bright-function` fires **once** per Tier-0 draft (R10) — after `missing_fields.length === 0` AND at least one `case_documents.processing_status === 'ready'` AND after the freshness-check extract pass.
...
- [ ] Layer 3 screen contains zero calls to `/api/edge/*` (R12). `grep -r '/api/edge' app/\(.*\)layer3\|contact-requests` should return nothing. No specialist card, no WhatsApp / LinkedIn CTAs anywhere in Layer 3.
...
- Uploads to Supabase Storage; inserts a row into `case_documents` with `processing_status = 'pending'`.
...
4. Upload a PDF. Confirm `case_documents.processing_status` transitions via Realtime (test under flaky network to prove the §8.2 fallback read works).
5. Upload a `.txt` file. Confirm the UI rejects it client-side (R7) — no request should reach the server.
6. Reach the Tier-0 draft. Confirm all three `case_narratives` rows exist **and** that temporarily deleting any one row (e.g. `tier0_srf_signal`) and refreshing still renders the other two without breaking the screen (R6).
7. Pay via Stripe test card `4242 4242 4242 4242`. Confirm within 2 s: `case_entitlements.plan = 'self_serve_report'`, a `jobs` row exists with `status='queued'`, then progresses to `running` → `completed`.
8. Watch the Layer 2 progress screen complete without user intervention (R9 — no call from the webhook to decision/report).
9. Try to access another user's `case_id` by hand-crafting a request to `/api/edge/extract`. Confirm it returns `404` (server-route ownership probe) and **never a 200** — proving RLS + server-route belt-and-braces (§9.2).
