...
- **Stripe catalogue correction:** the old SGD 99 Stripe price that previously backed Tier 1 is now the Human in the Loop Consultation price. Tier 1 is now **Basic Case Pack**, SGD 18, product key `self_serve_report`, env `STRIPE_PRICE_ID_SELF_SERVE_REPORT_SGD`, price `price_1TsLZdFp6sSKMUXXz5xEbxrA`. Tier 2 is **FIDREC Case Pack**, SGD 188, product key `fidrec_tier2_pack`, env `STRIPE_PRICE_ID_FIDREC_TIER2_PACK_SGD`, price `price_1TsLY5Fp6sSKMUXXdHFwRsny`. Consult is `human_consult_30m`, env `STRIPE_PRICE_ID_HUMAN_CONSULT_30M_SGD`, price `price_1SLOYUFp6sSKMUXXsTXlLdcT`.
