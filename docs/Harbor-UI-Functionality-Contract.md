...
| Checkout | Dashboard CTA plus `useCreateCheckoutSession` | Strict `{ caseId, productKey }` request; typed `GET /api/cases/:caseId/capabilities`; `PRODUCT_CATALOGUE`; Stripe webhook owns fulfilment | Browser goes to returned Stripe URL; Stripe returns to dashboard with success/cancel query state |
| Report lifecycle | Dashboard Layer 2 | Typed capability response, decision/report realtime hooks and job-status route; worker owns generation | UI displays running/drafting/ready/failed without inferring access from a redirect |
| Tier 2 | Dashboard Layer 3 | Server-gated case-pack JSON/export; Tier 2 checkout uses typed capability and eligibility authority | Consultation remains inactive; no promised outcome or SLA |
...
- Human consultation: legacy key `human_consult_30m` and product code `human_consult_99` are retained only to verify historic provider events; checkout is disabled and no consultation is allocated.

Prices, access and fulfilment come from `lib/payments/product-catalogue.ts` and server state. Report and Tier 2 CTAs consume the typed capability response; redirect query parameters are never proof of payment.

## Presentation rules

- New components receive authoritative data and callbacks; they do not query Supabase directly.
- Capability and billing refreshes use the typed case capability endpoint rather than reconstructing access from database tables.
