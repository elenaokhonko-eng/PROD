...
3. **Stripe catalogue correction (2026-07-12).** The old SGD 99 Stripe product/price that used to back Tier 1 has been renamed/reassigned to **Human in the Loop Consultation**. Do **not** keep using that price for Tier 1. The current catalogue is:
   - **Basic Case Pack** — **SGD 18** — product key `self_serve_report` — env `STRIPE_PRICE_ID_SELF_SERVE_REPORT_SGD` — Stripe price `price_1TsLZdFp6sSKMUXXz5xEbxrA`, product `prod_Us5ludfN8I4tv4`.
...
- Add or confirm the Stripe price for `self_serve_report` / Basic Case Pack at SGD 18, `fidrec_tier2_pack` / FIDREC Case Pack at SGD 188, and `human_consult_30m` / Human in the Loop Consultation at SGD 99.
