...
> **2026-08-27 planning update:** Harbor v3 integration and the evidence-based closure audit for Slices 0–8 are governed by [Harbor Redesign and Production Completion Plan](./Harbor-Redesign-Production-Plan.md). The status table below is historical and is not proof of current completion; status must be relocked from code, migrations, tests, and deployed-service evidence before redesign development starts.

**Status.** Slices 1–6 are validated locally on `Version-2.0-Refactor`; Slice 7 implementation/testing is in progress with Masha DB follow-up. **Slice 8** is now the Layer 3 / Tier 2 path: SGD 188 `fidrec_tier2_pack` plus separate SGD 99 `human_consult_30m`; see the 2026-07-12 correction below and **§10.7**.
...
### 2026-07-12 Slice 8 product correction

This section supersedes older Slice 8 wording that described the same surface as "SGD 99 / SGD 800" commerce.

1. **Tier 2 FIDReC pack is the case-linked product.** Price for the current build is **SGD 188**. It starts after the Tier 1 self-serve report exists and uses the already processed case material: evidence, extracts, narratives, decision/report output, and the existing FIDReC Tier-2 helper rows. The user receives an executive summary and chronology/timeline, with both **PDF** and **Markdown** downloads.
2. **`escalation_pack` is a valid post-report entitlement.** The state machine must treat `case_entitlements.plan = 'escalation_pack'` and/or `features.allow_escalation_pack = true` as a Layer 3 / Tier 2 ready state. It must not fall back to the Layer 1 -> Layer 2 buy-report flow.
3. **Stripe catalogue correction (2026-07-12).** The old SGD 99 Stripe product/price that used to back Tier 1 has been renamed/reassigned to **Human in the Loop Consultation**. Do **not** keep using that price for Tier 1. The current catalogue is:
   - **FI Pack** — **SGD 18** — product key `self_serve_report` — env `STRIPE_PRICE_ID_SELF_SERVE_REPORT_SGD` — Stripe price `price_1TsLZdFp6sSKMUXXz5xEbxrA`, product `prod_Us5ludfN8I4tv4`.
   - **FIDREC Case Pack** — **SGD 188** — product key `fidrec_tier2_pack` — env `STRIPE_PRICE_ID_FIDREC_TIER2_PACK_SGD` — Stripe price `price_1TsLY5Fp6sSKMUXXdHFwRsny`, product `prod_Us5j5HHbuf2SrE`.
   - **Human in the Loop Consultation** — **SGD 99** — product key `human_consult_30m` — env `STRIPE_PRICE_ID_HUMAN_CONSULT_30M_SGD` — Stripe price `price_1SLOYUFp6sSKMUXXsTXlLdcT`, product `prod_THyVNAxgk9VsgQ`; this is the renamed legacy SGD 99 price.
   - **Webhook endpoint** — `https://guidebuoyaisg.onrender.com/api/payments/webhook`.
4. **SGD 99 consult is separate from the packs.** It is a human advice/direction call for a claimant who wants help understanding the situation. It should be available on the same post-report surface, but it is not the Tier 1 or Tier 2 pack generator. Full integration waits on Masha's backend design: call recording, transcription, storage, and insertion into the case narrative set as a client consultation narrative.
5. **Stripe product keys for Slice 8.** Use explicit metadata/product keys instead of price inference alone: `self_serve_report` for FI Pack, `fidrec_tier2_pack` for FIDReC Pack, and `human_consult_30m` for the SGD 99 consult. Treat the older generic `STRIPE_PRICE_ID_SGD` env name as legacy/ambiguous and replace it with `STRIPE_PRICE_ID_SELF_SERVE_REPORT_SGD` in code/config.
6. **Reference test case.** Use Tier 2 case `688154e7-9cda-47ef-9cff-a27581766c3a` for Slice 8 QA once Masha confirms the hosted DB has the required Tier-2 artefacts.
...
5. **Layer 3 = Tier 2 (post–Tier-1) — one surface, three concerns: contact form + Tier 2 pack + consult.** In this app's schema, **"Layer 3" and "Tier 2" refer to the same post-Tier-1 stage** (after the self-serve **report** exists). The route/shell includes the human-in-the-loop FIDReC handoff via `POST /api/contact-requests`, the SGD 188 `fidrec_tier2_pack`, and the separate SGD 99 `human_consult_30m`. The Tier 1 product before this surface is now **FI Pack** at SGD 18 (`self_serve_report`). The contact path still has **no edge function** and still snapshots server-only fields (`user_id`, `case_id`, `amount_lost_sgd`, `financial_institution`). WhatsApp remains the existing root-layout `wa.me` link, no duplicate global widget, and no Clerk session required to open WhatsApp. **`escalation_pack` is a valid post-report entitlement and must not fall back to buy-report.**
6. **Post–Tier-1 FIDReC pack + human consult (Slice 8).** *Elaboration of the same surface as locked decision 5.* The case-linked **SGD 188** pack generates executive summary + chronology/timeline from existing processed case material and must be downloadable as PDF and Markdown. The **SGD 99** consult is a separate human advice/direction call; recording, transcription, storage, and insertion into case narratives as a client consultation narrative require Masha's backend workflow. **Engineering** — reuse the Tier-1 Stripe route/webhook pattern with explicit product metadata. **R9** preserved: neither add-on enqueues or runs the Layer 2 decision/report worker.
...
| 4B | Transition hooks (2 hooks: `useCreateCheckoutSession`, `usePaymentStatus`) | Done |
| 4C | Layer 2 hooks (4 hooks, includes `useJobStatus`) | Done |
| 4D | Layer 3 hook (`useSubmitContactRequest` — was `useSubmitWaitlist`) | Done |
| 5 | Wiring: driver hook, `dashboard-client.tsx` rewrite, new server routes (including **`/api/cases/bootstrap`** for R13 and **`/api/contact-requests`** for L3), Realtime provider, error boundaries | Done / verify E2E (§5 retest + E2E #1) |
...
| 8 | **Layer 3 / Tier 2:** SGD 188 FIDReC Tier 2 pack (`fidrec_tier2_pack`) with executive summary + chronology/timeline preview and PDF/Markdown downloads; separate SGD 99 human consult (`human_consult_30m`). **WhatsApp:** keep `app/layout.tsx`; L3 **copy** (see locked decision 5) | Planning / ready for Kimi implementation |
...
11. **2026-07-12 — Layer 3 = Tier 2, WhatsApp, and Slice 8 offers.** Confirm **[app/layout.tsx](../app/layout.tsx)** **persistent** `wa.me/6590727915` on **every URL**; **as anonymous visitor**, open link (**no login**). **R13:** no **Supabase** on pre-login hero. On **Layer 3 / Tier 2**, confirm on-page **Scam and Fraud Specialist** consult/Q&A **recommendation**. After Slice 8, confirm the SGD 188 `fidrec_tier2_pack` path and separate SGD 99 `human_consult_30m` path on the same surface if not in E2E #4.
...
### 10.7 Slice 8 current development plan - Tier 2 pack and consult split

**Goal.** Add the post-report Tier 2 path without disturbing the completed Tier 1 report path. The first Slice 8 deliverable is the **SGD 188 FIDReC Tier 2 pack**: executive summary + chronology/timeline generated from existing case material, previewed in the app, downloadable as PDF and Markdown. The **SGD 99 consult** remains visible as a separate human help offer; the call recording/transcript/narrative loop is a backend-dependent follow-up.

**Frontend work.**

- Update the state-machine driver so `escalation_pack` is a valid Layer 3 / Tier 2 entitlement and never routes the user back to the self-serve report checkout.
- On `L3-FormFilling` / post-report Layer 3, add a Tier 2 pack panel for eligible cases: price SGD 188, checkout CTA, entitlement state, generation/loading/error states, and ready state.
- Fetch `GET /api/fidrec/tier2/case-pack-json?caseId=...` after entitlement is present, then render the existing `submission_pack.executive_summary.narrative` and `submission_pack.chronology_of_events`.
- Add download actions for `format=pdf` and `format=md`. Markdown is required for AI compatibility.
- Keep the contact-request form and WhatsApp path intact. Add the SGD 99 consult CTA as a separate offer with clear copy that it is a human advice/direction call, not the automated case pack.

**Backend / Masha work.**

- Ensure the hosted DB for test case `688154e7-9cda-47ef-9cff-a27581766c3a` has the Tier 2 artefacts required by the existing FIDReC helpers: latest report/decision, processed evidence and chunks, case themes/findings/assertions, evidence requests/investigation questions, and extract/narrative data.
- Add or confirm the Stripe price for `self_serve_report` / FI Pack at SGD 18, `fidrec_tier2_pack` / FIDReC Pack at SGD 188, and `human_consult_30m` / Human in the Loop Consultation at SGD 99.
- Confirm the payment persistence schema supports explicit product keys/service types for `fidrec_tier2_pack` and `human_consult_30m`; `self_serve_report` may continue to persist as legacy `standard` service type if the webhook/RPC still expects that.
- In the Stripe webhook, `fidrec_tier2_pack` should set `case_entitlements.plan = 'escalation_pack'` and `features.allow_escalation_pack = true`, and **must not** enqueue a Layer 2 report job. `human_consult_30m` should persist the purchase/consult request but must not enqueue report generation either.
- For the consult follow-up, define the backend workflow for consent, call provider/session table, recording storage, transcription job, transcript retention, and the eventual `case_narratives` insert such as `narrative_type = 'client_consultation'` or the Masha-approved equivalent.

**Routes to implement or harden.**

- `POST /api/payments/create-checkout-session` accepts a known product key and writes Stripe metadata: `case_id`, `user_id`, `product_key`. It must use `STRIPE_PRICE_ID_SELF_SERVE_REPORT_SGD` for `self_serve_report`, not the renamed legacy SGD 99 consult price.
- `POST /api/payments/webhook` branches by `product_key` / price id and only enqueues the Tier 1 worker for the self-serve report product.
- `GET /api/fidrec/tier2/case-pack-json` should check ownership and `assertTier2Eligible(caseId)` before returning the pack.
- `GET /api/fidrec/tier2/case-pack-export?caseId=...&format=pdf|md` should check ownership and entitlement, then render the same pack to a downloadable PDF or Markdown attachment.
...
2. For each remaining slice in order (4A → 4B → 4C → 4D → 5 → 6 → 7 → **8** optional for Tier-2 post–Tier-1 marketplace; see §10.6):
   ...
   - **Also add (2026-07-12, does not replace the verbatim block):** *"Layer 3 = Tier 2. WhatsApp: **keep** `app/layout.tsx` `wa.me/6590727915` (no login; no duplicate FAB). L3: Scam and Fraud copy. R13: third-party on public. Slice 8: SGD 188 `fidrec_tier2_pack` plus separate SGD 99 `human_consult_30m`. See **locked decision 5–6**, **§10.7**."*
...
- **8 (L3 / Tier 2 commerce, §10.7):** With self-serve report completed, show SGD 188 `fidrec_tier2_pack` + separate SGD 99 `human_consult_30m`; **`checkout.session.completed`** branches correct; `escalation_pack` is Tier 2 ready, not buy-report; PDF/Markdown downloads work; **WhatsApp** = shell **(persistent)** + **L3 page** specialist line (R12: still no `/api/edge` in handoff **route**; see §10.2).
...
- **E2E #4 (after Slice 8, if in scope):** On **Layer 3 / Tier 2**, run the SGD 188 `fidrec_tier2_pack` checkout and the separate SGD 99 `human_consult_30m` checkout; verify **webhook** + **DB**; confirm no Layer 2 report job is enqueued for either add-on; confirm **WhatsApp** link/chat + **Scam and Fraud Specialist** copy visible; **`pnpm check:sm`** if script changed in Slice 8.
