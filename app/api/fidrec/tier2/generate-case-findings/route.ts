...
import { generateAndPersistCaseFindings } from "@/lib/server/fidrec/generate-case-findings"
...
const requestSchema = z.object({
  ...
  processedEvidenceJson: z.unknown(),
  customerNarrative: z.string().optional(),
})
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to generate case findings" }, { status: 500 })
  }
  ...
  try {
    const result = await generateAndPersistCaseFindings({
      caseId: parsedBody.caseId,
      processedEvidenceJson: parsedBody.processedEvidenceJson,
      customerNarrative: parsedBody.customerNarrative,
    })
