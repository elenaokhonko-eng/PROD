...
import { generateAndPersistCaseThemes } from "@/lib/server/fidrec/generate-case-themes"
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to generate case themes" }, { status: 500 })
  }
  ...
  try {
    const result = await generateAndPersistCaseThemes({ caseId: parsedBody.caseId })
    return NextResponse.json(result)
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to generate case themes"
