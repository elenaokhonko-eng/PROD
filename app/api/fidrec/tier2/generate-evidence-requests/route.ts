...
import { generateAndPersistEvidenceRequests } from "@/lib/server/fidrec/generate-evidence-requests"
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to generate evidence requests" }, { status: 500 })
  }
  ...
  try {
    const result = await generateAndPersistEvidenceRequests({ caseId: parsedBody.caseId })
