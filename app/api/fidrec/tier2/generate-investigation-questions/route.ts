...
import { generateAndPersistInvestigationQuestions } from "@/lib/server/fidrec/generate-investigation-questions"
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to generate investigation questions" }, { status: 500 })
  }
  ...
  try {
    const result = await generateAndPersistInvestigationQuestions({ caseId: parsedBody.caseId })
