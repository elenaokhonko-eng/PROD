...
/**
 * Pulls all output_text fragments from a Responses API payload, robustly.
 */ function extractOutputText(payload) {
  if (!payload || !Array.isArray(payload.output)) return "";
  return payload.output.flatMap((item)=>Array.isArray(item?.content) ? item.content : []).filter((c)=>c?.type === "output_text" && typeof c?.text === "string").map((c)=>c.text).join("").trim();
}
/**
 * Safety net: remove code fences and strip leading/trailing non-JSON.
...
async function callOpenAIResponses(input) {
  ...
  // Prefer JSON parsing; if OpenAI ever returns non-JSON, include raw for debugging.
  const payload = await res.json().catch(async ()=>{
    const raw = await res.text().catch(()=>"");
    throw new Error(`OpenAI extract failed: non-JSON HTTP response (${res.status}). Raw: ${raw}`);
  });
  if (!res.ok) {
    throw new Error(`OpenAI extract failed (${res.status}): ${JSON.stringify(payload)}`);
  }
  return payload;
}
/**
 * UPDATED: Robust OpenAI Responses API call using JSON mode + robust extraction.
 * Includes a single retry that asks the model to re-emit strict JSON only.
 */ async function openaiExtract(template, inputText) {
  ...
  // 1) First attempt in JSON mode (Responses API)
  const payload1 = await callOpenAIResponses({
    model: EXTRACT_MODEL,
    temperature: 0,
  ...
  // If JSON mode works, this should already be a JSON object string.
  // Keep sanitation anyway to survive edge cases.
  try {
    const jsonContent = sanitizeToJsonObject(outText);
    return JSON.parse(jsonContent);
  } catch (err1) {
    // 2) One retry: ask it to output STRICT JSON only, no commentary.
    // Feed back the raw output so it can correct formatting.
    const retryPrompt = "The previous output was NOT valid JSON.\n" + "Re-output ONLY a valid JSON object that matches the TEMPLATE EXACTLY.\n" + "No markdown. No commentary. No trailing commas. No extra keys.\n\n" + `TEMPLATE (JSON):\n${JSON.stringify(template)}\n\n` + `PREVIOUS OUTPUT (fix formatting only; do not add new info):\n${outText}\n`;
    const payload2 = await callOpenAIResponses({
      model: EXTRACT_MODEL,
      temperature: 0,
      text: {
        format: {
    ...
    try {
      const jsonContent2 = sanitizeToJsonObject(outText);
      return JSON.parse(jsonContent2);
    } catch (err2) {
      console.error("OpenAI extract parse failed. Attempt1 error:", err1?.message ?? err1);
      console.error("OpenAI extract parse failed. Attempt2 error:", err2?.message ?? err2);
      console.error("OpenAI extract raw output (attempt2):", outText);
      throw new Error(`OpenAI extract: model did not return valid JSON after retry. ` + `Attempt2 error: ${err2?.message ?? String(err2)}`);
    }
  }
}
serve(async (req)=>{
