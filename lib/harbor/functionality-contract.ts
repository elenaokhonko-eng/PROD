...
export const HARBOR_FUNCTIONALITY_CONTRACT: Record<HarborFlowId, HarborFlowContract> = {
  'narrative-capture': {
    ...
    hooks: ['persistPendingNarrative', 'readPendingNarrative'],
    api: [
      'POST /api/transcribe',
    ...
    redirects: ['/sign-up?redirect_url=/onboarding', '/app/case/:caseId/dashboard'],
    constraints: ['The public CTA never creates router sessions; sign-in/sign-up hands the local draft to authenticated POST /api/cases/bootstrap.'],
  },
  'evidence-upload': {
  ...
  checkout: {
    ...
    api: ['GET /api/cases/:caseId/capabilities', 'POST /api/payments/create-checkout-session'],
    dataOwners: ['typed case capability response', 'PRODUCT_CATALOGUE', 'Stripe webhook fulfilment'],
    analytics: [],
  ...
  'report-lifecycle': {
    ...
    api: ['GET /api/cases/:caseId/capabilities', 'GET /api/cases/:caseId/job-status'],
    dataOwners: ['typed case capability response', 'case_decision_runs realtime query', 'reports realtime query', 'worker-owned jobs'],
    analytics: [],
    redirects: [],
